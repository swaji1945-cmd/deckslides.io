    ...
    <>
      ...
      <div className="admin-main">
        <div className="admin-section-head">
          <h1>Blog posts</h1>
          <Link href="/light/admin/blog/new" className="admin-btn admin-btn--primary">+ New post</Link>
        </div>

        {posts.length === 0 ? (
          <div className="admin-empty">
            <p>No posts yet.</p>
            <Link href="/light/admin/blog/new" className="admin-btn admin-btn--primary" style={{ marginTop: 16 }}>+ Write your first post</Link>
          </div>
        ) : (
          <table className="admin-table">
            ...
            <tbody>
              {posts.map((p) => (
                <tr key={p.id}>
                  <td>
                    <Link href={`/light/admin/blog/${p.id}`} style={{ color: "inherit", textDecoration: "none" }}>
